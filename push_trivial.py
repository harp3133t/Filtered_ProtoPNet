...
def push_prototypes(dataloader, # pytorch dataloader (must be unnormalized in [0,1])
    ...
    start = time.time()
    prototype_shape = prototype_network_parallel.module.trivial_prototype_shape
    n_prototypes = prototype_network_parallel.module.trivial_num_prototypes
    ...
    prototype_network_parallel.module.prototype_vectors_trivial.data.copy_(torch.tensor(prototype_update, dtype=torch.float32).cuda())
...
def update_prototypes_on_batch(search_batch_input,
    ...
    with torch.no_grad():
        ...
        protoL_input_torch, proto_dist_torch = prototype_network_parallel.module.push_forward_trivial(search_batch)
    ...
    for j in range(n_prototypes):#980
        #if n_prototypes_per_class != None:
        if class_specific:
            # target_class is the class of the class_specific prototype
            target_class = torch.argmax(prototype_network_parallel.module.trivial_prototype_class_identity[j]).item()
            ...
            if dir_for_saving_prototypes is not None:
                ...
                if prototype_img_filename_prefix is not None:
                    ...
                    plt.imsave(os.path.join(dir_for_saving_prototypes,
                                            str(j) + prototype_img_filename_prefix + '-original' + '.png'),
                               original_img_j,
                               vmin=0.0,
                               vmax=1.0)
                    ...
                    if rf_img_j.shape[0] != original_img_size or rf_img_j.shape[1] != original_img_size:
                        plt.imsave(os.path.join(dir_for_saving_prototypes,
                                                str(j) + prototype_img_filename_prefix + '-receptive_field' + '.png'),
                                   rf_img_j,
                                   vmin=0.0,
                                   vmax=1.0)
                        overlayed_rf_img_j = overlayed_original_img_j[rf_prototype_j[1]:rf_prototype_j[2],
                                                                      rf_prototype_j[3]:rf_prototype_j[4]]
                        plt.imsave(os.path.join(dir_for_saving_prototypes,
                                                str(j) + prototype_img_filename_prefix + '-receptive_field_with_self_act' + '.png'),
                                   overlayed_rf_img_j,
                                   vmin=0.0,
                                   vmax=1.0)
                    ...
                    plt.imsave(os.path.join(dir_for_saving_prototypes,
                                            str(j) + prototype_img_filename_prefix + '.png'),
                               proto_img_j,
                               vmin=0.0,
                               vmax=1.0)
