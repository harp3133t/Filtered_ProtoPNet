...
def auc(arr):
    ...
    # return (arr.sum() - (arr[0] + arr[-1])) / (arr.shape[0] - 2)
...
class CausalMetric():
    ...
    def single_run(self, img_tensor, explanation, verbose=0, save_to=None):
        ...
        for i in range(n_steps+1):
            ...
            if i < n_steps:
                ...
                start_np[0, :, coords] = finish.cpu().numpy().reshape(1, 3, HW)[0, :, coords]
                start_np = start_np.reshape(1, 3, H, W)
    ...
    def evaluate(self, img_batch, exp_batch, batch_size):
        ...
        for i in tqdm(range(n_samples // batch_size), desc='Predicting labels'):
            preds = self.model(img_batch[i*batch_size:(i+1)*batch_size].cuda()).detach().cpu().numpy()
