...
def _training(model, dataloader, optimizer=None, class_specific=True, use_l1_mask=True, coefs=None, 
log=print, wandb_logger = None):
    '''
    ...
    wandb_logger.log({"train Acc": n_correct / n_examples * 100})
    wandb_logger.log({"train loss":total_loss})
    return n_correct / n_examples, results_loss


def _testing(model, dataloader, optimizer=None, class_specific=True, use_l1_mask=True, coefs=None, 
log=print, wandb_logger = None):
    '''
    model: the multi-gpu model
    dataloader:
    ...
    wandb_logger.log({"Test Acc": n_correct / n_examples * 100})
    return n_correct / n_examples, results_loss


def train(model, dataloader, optimizer, class_specific=False, coefs=None, log=print, wandb_logger = None):
...
def test(model, dataloader, class_specific=False, log=print, wandb_logger = None):
    log('\ttest')
    model.eval()
